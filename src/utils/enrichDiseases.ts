  ...
  switch (cluster) {
    ...
    case 'postpartum':
      return [
        ...
        'УЗИ молочных желез при подозрении на лактостаз, мастит или абсцесс.',
...
const buildGynecologyImaging = (cluster: GynecologyCluster) => {
  switch (cluster) {
    ...
    case 'breast':
      return [
        'УЗИ молочных желез с оценкой формы, контуров, эхоструктуры и подмышечных лимфоузлов.',
        'Маммография или томосинтез по возрасту и онкориску.',
        'МРТ молочных желез при высокой плотности ткани, генетическом риске или неясной картине.',
      ];
    case 'menopause':
      return [
  ...
  switch (cluster) {
    ...
    case 'menopause':
      return {
        ...
        acog: 'ACOG рекомендует индивидуально назначать HT/MHT женщинам моложе 60 лет или в пределах 10 лет от менопаузы при благоприятном benefit-risk профиле, с ежегодным пересмотром. Любое постменопаузальное кровотечение требует исключения патологии эндометрия до или на фоне терапии.',
    ...
    case 'vulvar':
      return {
        ...
        ranzcog: 'Австралийские рекомендации сходятся с европейскими: ранняя биопсия, длительное наблюдение за рубцующимися дерматозами и осторожное отношение к позднему распознаванию dVIN/рака вульвы. При сексуально-болевых нарушениях рекомендован multidisciplinary pelvic pain подход.',
  ...
  switch (cluster) {
    ...
    case 'menopause':
      return [
        ...
        'При кровянистых выделениях сначала исключать патологию эндометрия, затем решать вопрос о ЗГТ.',
    ...
    case 'vulvar':
      return [
        ...
        'Избегать агрессивных местных раздражителей и подробно разбирать уход за вульвой.',
  ...
  switch (cluster) {
    ...
    case 'menopause':
      return {
        ...
        tests: ['Оценка симптомов и переносимости терапии', 'ТВУЗИ при кровянистых выделениях или факторах риска', 'Контроль молочных желез, костной ткани и метаболического профиля'],
      };
    default:
      return {
...
const buildGynecologyCases = (cluster: GynecologyCluster, disease: Disease) => {
  switch (cluster) {
    case 'oncology':
      return [
        {
          presentation: `Пациентка обратилась с симптомами, подозрительными на ${disease.name.toLowerCase()}, и прогрессирующим ухудшением состояния за последние месяцы.`,
...
export const enrichDisease = (rawDisease: DiseaseInput): Disease => {
  ...
  const gynecologyGuidelines = disease.subtitle === 'Гинекология' ? buildGynecologyGuidelines(getGynecologyCluster(disease)) : null;
